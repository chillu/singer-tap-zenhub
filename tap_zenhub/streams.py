...
def sync_issues_for_repo(repo, last_updated, github_client, zenhub_client):
    owner, name = repo.split('/')
    all_issues = []
    ...
    if last_updated:
        LOGGER.info(
            repo + ': Get closed Github issues since ' + last_updated)
    else:
    ...
    for github_issue in github_issues:
        LOGGER.info(repo + ': Get Zenhub issue for #' +
...
def sync_issues(config, state):
    ...
    repos_by_last_updated = sorted(
        repos_by_last_updated.items(),
